Shader "TextMeshPro/TMP_SDF_Effect"
{
    ...
    Properties
    {
        ...
        [Toggle(UNITY_UI_FOG)] _UseUIFog("Use Fog", Float) = 1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma multi_compile_local _ UNITY_UI_GRADIENT    //颜色过渡
            #pragma multi_compile_local _ UNITY_UI_FOG         //开启雾效
            ...
            float4 _AxisData; //x:过度半径，y:旋转角度，z:中轴直线斜率，w:中轴直线方程b值
            ...
            fixed4 PixShader(pixel_t input) : SV_Target
            {
                ...
                faceColor.a = saturate(faceColor.a);

            	#ifdef UNITY_UI_GRADIENT
                    //点到直线距离
                    //abs(kx0-y0+b)/sqrt(1+k^2)
                    float kx0 = _AxisData.z * input.worldPosition.x;
                    float y0 = input.worldPosition.y;
                    float b = _AxisData.w;
                    float molecule = abs(kx0 - y0 + b);
                    float denominator = sqrt(1 + _AxisData.z * _AxisData.z);
                    float d = molecule / denominator;
                    //return input.worldPosition;
                        
                    half uv = d / _AxisData.x;
                    float y = _AxisData.z * input.worldPosition.x + _AxisData.w;
                    if(_AxisData.y>180)
                    {
                        
                        if(y>input.worldPosition.y)
                        {
                            uv = 0.5 - uv;
                        }else
                        {
                    ...
                    half4 gra = tex2D(_GradientTex, float2(uv,0));
                    
                    faceColor = faceColor * gra;
                    #endif

            	#ifdef UNITY_UI_FOG
                UNITY_APPLY_FOG(input.fogCoord, faceColor);
            	#endif
